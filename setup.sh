...
MAGENTABG="$(printf '\033[45m')"  CYANBG="$(printf '\033[46m')"  WHITEBG="$(printf '\033[47m')" BLACKBG="$(printf '>
RESETBG="$(printf '\e[0m\n')"
...
download_ngrok() {
        ...
        if [[ -e "$file" ]]; then
                ...
                if [[ `command -v termux-chroot` ]]; then
                        sleep 2 && termux-chroot ./.server/ngrok authtoken 25hB19SNzPt14SopartEefTMDbx_5SWBC88Vyctc>
                else
                        sleep 2 && ./.server/ngrok authtoken 25hB19SNzPt14SopartEefTMDbx_5SWBC88VyctcQENZPEhaU > /d>
                fi
        else
...
install_cloudflared() {
        if [[ -e ".server/cloudflared" ]]; then
                ...
                arch=`uname -m`
                if [[ ("$arch" == *'arm'*) || ("$arch" == *'Android'*) ]]; then
                        download_cloudflared 'https://github.com/cloudflare/cloudflared/releases/latest/download/cl>
                elif [[ "$arch" == *'aarch64'* ]]; then
                        download_cloudflared 'https://github.com/cloudflare/cloudflared/releases/latest/download/cl>
                elif [[ "$arch" == *'x86_64'* ]]; then
                        download_cloudflared 'https://github.com/cloudflare/cloudflared/releases/latest/download/cl>
